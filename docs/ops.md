...
|---|---|---|
| `HTTPS_PROXY`, `https_proxy` | `--onecli-gateway-url` with this repo's **agent identity** access token folded in as userinfo | **No.** It is a credential. On a `container`-runner run it is seeded into the pane environment by tmux `-e` and forwarded into the container by NAME only (`--env K`), exactly like `LAB_TOKEN` — so the value never appears in the podman argv, in `ps` output for the pane, or in podman's logs. |
...
| `onecli gateway: the agent identity for repo <repo> carries no access token in OneCLI's agent listing; this OneCLI build's wire shape differs from the one verified in internal/onecli/wire.go` | The agent resolved, but its listing row held no `accessToken` — a OneCLI build whose wire shape drifted from the verified one. | Check the sidecar's version against the one the deployment pins; a wire drift is corrected in `internal/onecli/wire.go`. |
...
**The token is stable, and lab restarts are harmless to it.** A repo's agent access token is *read* from OneCLI's agent listing at every spawn — lab never calls the regenerate endpoint, so nothing lab does can invalidate the token a running run holds, and a lab restart changes nothing (there is no in-process token state to lose). The one thing that does invalidate it is regenerating it **in OneCLI's own dashboard**: do that while wired runs of the repo are live and their proxied calls fail per request with a proxy auth error until the runs are stopped and re-started, while everything on their `NO_PROXY` list — the tracker, `git push`, the model stream — is unaffected.
...
That proves the sidecar answers. It does **not** prove injection — the health probe deliberately opens a TCP connection and stops there, because a `CONNECT` probe would have to spend an agent's access token, and spending a per-repo credential on a health check is how health checks start causing outages. Injection is proved once, live, from inside a run:
...
Pull requests are gated by **GitHub Actions**, on GitHub's hosted `ubuntu-latest` runners (ADR-0023). Two gates run on PRs, two more are path-gated — one to the container images, one to the docs site — and the deploy, the only workflow still on Forgejo, polls from the other side (below).
...
**Docs gate** ([`.github/workflows/docs.yml`](../.github/workflows/docs.yml), jobs `build` and `deploy`, [ADR-0066](adr/0066-docs-site-github-pages.md)) — renders the documentation site and, on a release tag, publishes it to GitHub Pages at `https://cloonar.github.io/coding-lab/`. It is path-gated on `pull_request` to everything that can change the render (`docs/**`, `mkdocs.yml`, `mkdocs_hooks.py`, the injected root files `README.md`/`CONTEXT.md`/`CONTRIBUTING.md`, `flake.nix`, `flake.lock`, and the workflow itself) and runs unconditionally on a `v*` tag push or `workflow_dispatch`. It is **not** a required status check and deliberately has no no-op twin: a PR matching none of those paths simply gets no `docs` result, so nothing hangs at "Expected". What it needs:

- **`build`** is the hermetic gate's prerequisites, minus the closure: the same Determinate installer at default daemon init and the same egress for `install.determinate.systems`, `cache.nixos.org` and `github.com` (flake inputs, and `actions/checkout@v7`), but it enters `nix develop .#docs` — a devshell holding only the mkdocs toolchain — so it realizes Python + mkdocs + Material and neither the Go toolchain nor node. No `proxy.golang.org`, no `registry.npmjs.org`. The command is `mkdocs build --strict`, which is the gate itself: without `--strict` mkdocs reports broken internal links and nav problems as warnings and exits 0.
- **`build` holds `contents: read` and references no `secrets.*`** — it is the leg a fork PR executes, so it is incapable of publishing by construction. It uploads the rendered `site/` as a Pages artifact on every trigger (the PR leg included, so a PR rehearses the full deploy payload), which needs GitHub's hosted actions artifact backend reachable.
- **`deploy`** (`needs: build`, `if`-gated to tag pushes and dispatches, so no `pull_request` can reach it) is the **only** job holding `pages: write` and `id-token: write`. Beyond the artifact backend it needs GitHub's Pages deployment API and the Actions OIDC token endpoint — `actions/deploy-pages@v5` mints an OIDC token and Pages authenticates the deployment with it, so there is no operator-provisioned secret anywhere in this workflow. It takes no checkout.
- **One-time admin step, which no token can perform:** GitHub Pages must be enabled for the repository with its source set to **"GitHub Actions"** (Settings → Pages). Until a maintainer does that, `deploy` fails loudly inside `actions/deploy-pages` rather than silently no-opping — a red tag run is discoverable, a green run that published nothing is not.
