...
| `container.enable` | `false` | Master switch for container-runner host provisioning (see [Container runner](#container-runner)) — gates **all** the host mutations below, never their non-emptiness. Turns on `virtualisation.podman.enable` (crun + `policy.json`/`registries.conf`), puts podman + passt on the unit PATH, sets `serviceConfig.Delegate = true`, provisions the preserved runtime dir (`RuntimeDirectory=lab` + `XDG_RUNTIME_DIR=/run/lab`), and adds the service user's subuid/subgid ranges — everything the container preflight verifies, in one option. Off by default: a host that never uses `repos.runner = container` provisions none of it. |
| `container.toolsImages` | `{ }` | `attrsOf nonEmptyStr` keyed by **lab provider ID**, rendered into `--container-tools-image provider=ref[,provider=ref…]` (comma-joined) ahead of `extraFlags` — the `@sha256`-pinned agent-tools refs ([Agent-tools images](#agent-tools-images), [Container runner](#container-runner)). Enabling `container.enable` with this **empty fails eval** (a container host with no tools image can spawn nothing). Keys are **not** validated against registered provider IDs at eval — the server's boot error names the registered IDs — and digest-pinning is documented, not enforced. |
...
**On NixOS the module provisions all of it** through `services.lab.container.*` ([option table](#nixos-module-recommended) above) — one `enable` plus `toolsImages` and you are done. Enabling it turns on rootless podman, puts podman + passt on the unit PATH, delegates the cgroup, provisions the preserved runtime dir, and adds the service user's subuid/subgid ranges; the bullets below are what that provisioning *amounts to* (and what a non-NixOS host assembles by hand):
