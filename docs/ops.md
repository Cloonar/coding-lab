...
| `container.enable` | `true` | Master switch for container-runner host provisioning (see [Container runner](#container-runner)) — gates **all** the host mutations below, never their non-emptiness. Turns on `virtualisation.podman.enable` (crun + `policy.json`/`registries.conf`), puts podman + passt on the unit PATH, sets `serviceConfig.Delegate = true` + `DelegateSubgroup = "main"` and installs the `lab-cgroup-hygiene` pre-start oneshot (the restart-safe cgroup layout, ADR-0058), provisions the preserved runtime dir (`RuntimeDirectory=lab` + `XDG_RUNTIME_DIR=/run/lab`), and adds the service user's subuid/subgid ranges — everything the container preflight verifies, in one option. **On by default** (ADR-0054): enabling lab makes the host container-ready out of the box. Set `false` for a host-only deployment — the unit is then byte-identical to the pre-container output (guarded by the `nixos-module` zero-diff check). Note an operator-forced `virtualisation.podman.enable = false` now conflicts at eval unless container mode is disabled too. |
...
- **cgroup v2 with the restart-safe delegated-subgroup layout ([ADR-0058](adr/0058-restart-safe-container-cgroup-layout.md)).** Container panes run `podman --cgroup-manager=cgroupfs run … --cgroup-parent=<lab's unit cgroup>/payload`: every container lands in a lab-owned `payload/` subtree *next to* the `main/` subgroup systemd spawns lab into, rather than podman's rootless default of `user.slice` — so the `--memory`/`--pids-limit` caps take effect in the exact subtree preflight checks. Two systemd directives make it work, both set by `container.enable`: `Delegate=yes` (chowns the unit cgroup to the service user) and `DelegateSubgroup=main` (systemd ≥ 254; keeps the delegated root process-free — cgroup v2 forbids processes directly in a cgroup that delegates controllers, and without the subgroup the **first restart after any container ran** fails with `Failed to spawn executor: Device or resource busy`, the 2026-07-25 outage). The module also installs the root `lab-cgroup-hygiene` oneshot, run before every lab start, which clears any controller delegation something re-armed inside `main/` (podman's move-to-a-`runtime/`-subgroup workaround is the known culprit) — restarts stay safe unconditionally. At startup lab adopts stray root processes into its own cgroup, enables `+memory +pids` on the unit root, and creates `payload/`; preflight verifies the layout (lab in a delegated subgroup — write access to the unit root, what `Delegate=yes` grants — and memory+pids actually present in `payload/`, never just listed on some unchecked subtree), and a **restart-safety guard** re-checks the layout before every container spawn, hard-refusing spawns rather than letting the next deploy's restart fail. On an older systemd the unknown `DelegateSubgroup` directive is ignored and preflight refuses container spawns with the actionable `cgroup-layout` failure.
