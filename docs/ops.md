...
- **cgroup v2 with memory+pids delegated to the lab unit.** Container panes run `podman run … --cgroups=split`, which places the container's cgroup *inside lab's own* (`<lab's cgroup>/libpod-payload-<id>`) rather than podman's rootless default of `user.slice` — so the `--memory`/`--pids-limit` caps take effect in the exact subtree preflight checks. That only works if lab's cgroup is delegated: boot on the unified hierarchy and set `Delegate=yes` on the lab systemd unit (NixOS: `systemd.services.lab.serviceConfig.Delegate = true;`). Preflight verifies delegation by write access to lab's cgroup directory (what `Delegate=yes` grants the service user), not just by the presence of the controllers — a non-delegated unit still lists `memory pids` in `cgroup.controllers` but cannot create a limited child, which would leave the caps *silently absent*, so preflight fails instead.
...
- **The image knobs** (flags table above): `--container-image` is the global **default** dev image sessions run in — **optional** now that each repo can set its own **Dev image** ref (`repos.image_ref`, **Dev image expectations** below, ADR-0053). A repo carrying its own ref needs no global default, and preflight no longer refuses on an unset global (a spawn with no effective image is refused at spawn instead). `--container-tools-image provider=ref[,provider=ref…]` carries the `@sha256`-pinned agent-tools refs from the release job ([Agent-tools images](#agent-tools-images), ADR-0051) and — unlike the dev-image knob — *is* preflight-checked: the registry must be reachable, and preflight resolves every configured tools ref (`podman image exists`, else one pull).
