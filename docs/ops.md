...
| `container.enable` | `true` | Master switch for container-runner host provisioning (see [Container runner](#container-runner)) — gates **all** the host mutations below, never their non-emptiness. Turns on `virtualisation.podman.enable` (crun + `policy.json`/`registries.conf`), puts podman + passt on the unit PATH, installs the never-restarted `lab-payload.service` holder unit that owns the delegated payload cgroup plus the `lab-cgroup-hygiene` pre-start oneshot (the restart-safe cgroup layout, ADR-0059, superseding ADR-0058 — `lab.service` itself stays plain and undelegated), provisions the preserved runtime dir (`RuntimeDirectory=lab` + `XDG_RUNTIME_DIR=/run/lab`), and adds the service user's subuid/subgid ranges — everything the container preflight verifies, in one option. **On by default** (ADR-0054): enabling lab makes the host container-ready out of the box. Set `false` for a host-only deployment — the unit is then byte-identical to the pre-container output (guarded by the `nixos-module` zero-diff check). Note an operator-forced `virtualisation.podman.enable = false` now conflicts at eval unless container mode is disabled too. |
...
- **cgroup v2 with the restart-safe holder-unit layout ([ADR-0059](adr/0059-payload-cgroup-holder-unit.md), superseding [ADR-0058](adr/0058-restart-safe-container-cgroup-layout.md)).** Container panes run `podman --cgroup-manager=cgroupfs run … --cgroup-parent=/system.slice/lab-payload.service/payload`: every container lands in the `payload/` subtree of a dedicated **holder unit**, `lab-payload.service`, rather than podman's rootless default of `user.slice` — so the `--memory`/`--pids-limit` caps take effect in the exact subtree preflight checks. The holder is a tiny `sleep infinity` unit (`Type=exec`, `User`=the lab service user, `Delegate=yes`, `DelegateSubgroup=main`, `KillMode=process`) whose delegated root carries `+memory +pids` on its `subtree_control`; it is **never restarted** (NixOS `restartIfChanged=false`), and that is the whole point — the executor-spawn trap only fires on a *restart* into a controller-armed cgroup root, so a unit that never restarts can hold a delegating root indefinitely. `lab.service` itself is now **plain and undelegated**: it, the tmux server, and every host process live directly in its own cgroup, whose `subtree_control` must stay **empty forever**. The moment a controller is enabled on `lab.service`'s own root and any session survives a stop (`KillMode=process`, the product feature), the next `lab.service` restart fails to spawn its executor with `Failed to spawn executor: Device or resource busy` — the 2026-07-25 *and* 2026-07-26 outages: `DelegateSubgroup=` does **not** move a unit's main-process spawn off the cgroup root (systemd honors it only for the executor's own later self-migration), which is why ADR-0058's in-unit `main/`+`payload/` split validated green once and then wedged on the next deploy (see ADR-0059 for the systemd source receipt). `container.enable` wires `lab.service` with `Wants=`/`After=` the holder (`Wants`, not `Requires`: a missing holder degrades to an actionable preflight refusal, never a vetoed lab start) and installs the root `lab-cgroup-hygiene` oneshot, run before every lab start, which clears every `cgroup.subtree_control` under `/system.slice/lab.service` — *including the unit root's own* — bottom-up and prunes empty child dirs. That oneshot is both the migration off the ADR-0058 layout (the first post-upgrade start still finds the old delegated root armed) and standing insurance against anything re-arming the undelegated unit between restarts. At startup lab's preflight (`podmanx.setupCgroups`) targets the hardcoded holder path: it confirms the holder root is writable by the service user (the `Delegate=yes` + `User=` chown), adopts any stray holder-root process into the holder's `main/` (covering a systemd < 254 that ignores `DelegateSubgroup=`), enables `+memory +pids` on the holder root, creates `payload/`, and verifies the controllers actually arrived there — never just listed on some unchecked subtree. If the holder is **absent or undelegated**, preflight refuses container spawns with the **`cgroup-holder`** check naming `lab-payload.service` as the unit to bring up (this replaces ADR-0058's retired `cgroup-layout` check); the **`cgroup-delegation`** check covers the controllers-present-in-`payload/` verdict, and a **restart-safety guard** (**`cgroup-restart-safety`**) re-checks the two live invariants — `lab.service`'s own `subtree_control` empty, and `payload/` still carrying memory+pids — before every container spawn, hard-refusing rather than letting the next deploy's restart fail.
