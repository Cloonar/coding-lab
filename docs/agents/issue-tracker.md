...
- **Pull requests**: managed with `labctl pr ...` (`create`, `view`, `list`, `checks`, `merge`, plus the review verbs `reject` / `rerequest` / `approve` / `comment`). The `land-pr` skill drives the Forgejo PR flow.
