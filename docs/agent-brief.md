...
`GET /issue` (the run's claimed issue, with comments) · `GET /issues/{n}` · `POST /issues/{n}/comments` · `POST /prs` {title, body} (routes to forge PR or built-in CR; server injects/validates `Closes #N`; **incogni sanitization happens here**).

### 8.3 `labctl`

Reads `LAB_URL`/`LAB_TOKEN` from env. Commands: `labctl issue view [n]`, `labctl issue list`, `labctl issue comment <n> <body>`, `labctl pr create --title … --body …`. Plain, parseable output; exit codes meaningful. Ship as a second small binary from the same module, present on every session's PATH.
