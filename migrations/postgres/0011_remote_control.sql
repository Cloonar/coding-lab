-- 0011_remote_control — remote-control as a layered spawn knob (postgres dialect).
...
-- how they were actually spawned. DEFAULT FALSE is what lets the NOT NULL column
-- be added to a non-empty table at all (both dialects reject a bare NOT NULL
-- ADD COLUMN there); the explicit UPDATE then states the truth for the existing
-- rows. New rows are stamped by the resolver, so the default is never the value
-- that matters.
--
-- Identical to the sqlite dialect — BOOLEAN and the TRUE/FALSE literals are
-- spelled the same in both, so no statement here diverges.
