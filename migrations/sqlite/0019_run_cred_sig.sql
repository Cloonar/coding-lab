-- 0019_run_cred_sig — the runs.cred_sig adopt-check baseline (sqlite dialect;
-- issue #222). Server-owned credential authority: lab's rotation loop injects
-- rotated provider credentials into every live run's instance HOME, and
-- cred_sig is the opaque signature (existence+mtime+size digests — never
-- token contents) lab last injected there. NULL means "never stamped":
