...
**Agent identity**:
The OneCLI agent object lab maps one-to-one onto a lab **repo**: the unit the **credential gateway** authenticates (a per-agent token in the `Proxy-Authorization` header) and authorizes (its **grants**). One OneCLI project for the whole lab, one agent per repo, created lazily and idempotently by name so concurrent spawns converge on one object rather than duplicating it (ADR-0067). Not a lab **run token** — that is lab's own tracker-scoped credential, `lab_run_…` — and not a **provider**: a OneCLI agent is a credential-scoping identity, while "agent" everywhere else in this repo means the coding agent; see **Flagged ambiguities**.
...
- A repo's secrets reach its **instances** only as **grants** on that repo's **agent identity**, never as values materialized in the instance itself; the **master key**/vault path for git credentials (ADR-0006) is unchanged and stays independent of the **credential gateway**.
- The **credential gateway** is fail-closed at spawn (enforced starting in a later epic): an unreachable sidecar refuses the run rather than starting it credential-less.
