...
**Agent identity**:
The OneCLI agent object lab maps one-to-one onto a lab **repo**: the unit the **credential gateway** authenticates (a per-agent token in the `Proxy-Authorization` header) and authorizes (its **grants**). One OneCLI project for the whole lab, one agent per repo, addressed by its `identifier` — the slug `onecli.AgentIdentifier` derives from the repo's store ID (`repo_<32 hex>` → `repo-<32 hex>`), unique per project and immutable upstream, so it is the match key every call site resolves by, exactly and never by name or pattern, and the **grants** hold still across a rename on either side. Its `name` is the opposite: a display string carrying the repo's human name, owned and overwritten by lab, so the dashboard reads as repo names and a rename typed there survives only until the next ensure. Ensuring is idempotent and safe under concurrency (the loser of a 409 race re-lists and returns the winner), creates the agent eagerly when the repo is created and heals a stale name in place, with the ensures at startup, at spawn, and at grant-attach as backstops for repos that predate the gateway configuration; startup's pass is one-way — it creates and renames, never deletes, because the OneCLI project is surface other tools share — and deleting the repo deletes the agent, its **grants** cascading away while the project's pool of secrets and connections is untouched. Every one of those writes is best-effort, degrading to a warning and a stale name or an orphaned agent, and never to a delete-and-recreate, which would destroy the grant set and mint a token every in-flight run of that repo would 401 on. Its proxy token rides in the run's `HTTPS_PROXY` as userinfo, which makes that variable a credential (forwarded by name through tmux, never in a podman argv), and lab reads it as the stable `accessToken` already carried on the agent listing `EnsureAgent` fetches rather than minting or regenerating one — no per-process cache, no cross-spawn lock, and no restart residual, since a run that outlives a lab restart keeps a token the gateway still honours (ADR-0067). Not a lab **run token** — that is lab's own tracker-scoped credential, `lab_run_…` — and not a **provider**: a OneCLI agent is a credential-scoping identity, while "agent" everywhere else in this repo means the coding agent; see **Flagged ambiguities**.
_Avoid_: service account, principal, client, identity (bare), agent (bare, in secret contexts)

**Dashboard exposure**:
The operator-configured way OneCLI's own dashboard — the **credential gateway** sidecar's Next.js app, on the same 10254 dashboard/API port lab's REST client dials — is reached through lab's auth, one of exactly three modes. `off` (the default) exposes nothing and leaves an SSH tunnel as the access path; `port` has lab open a second listener and whole-origin reverse-proxy the dashboard behind its own session gate, bouncing an unauthenticated browser navigation through lab's login on a fixed `next` keyword plus a path — never a URL, so the round trip cannot become an open redirect — and needing no cookie configuration at all, because cookies are host-scoped and not port-scoped; `subdomain` has the operator's own reverse proxy front `onecli.<domain>` and delegate auth to lab's `GET /api/v1/auth/check`, which additionally needs `--session-cookie-domain` to widen lab's host-only session cookie to the parent domain (a deliberate trade: every host under that domain then receives it). Reported to the web UI as a mode plus a browser-facing URL at `GET /api/v1/onecli/dashboard` — `url` omitted when off — so no consumer has to know the topology. The dashboard stays the only surface anywhere where secret and connection *values* are created or edited: lab has no such screen and grows none by design, and its own screens stop at **grants** (ADR-0067). Not a path prefix under lab's own origin — that shape was rejected outright (OneCLI ships no `basePath`), and `port` and `subdomain` are whole origins.
_Avoid_: dashboard proxy, subpath proxying, OneCLI UI mode, admin panel, embedded dashboard
