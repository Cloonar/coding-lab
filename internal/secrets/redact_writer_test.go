...
// redactChunks runs input through a fresh RedactingWriter built from values, writing
...
func redactChunks(t *testing.T, values map[string]string, input string, sizes []int) string {
	...
	r := NewRedactingWriter(&buf, NewMatcher(values))
	data := []byte(input)
...
func TestRedactingWriter_EachForm(t *testing.T) {
...
func TestRedactingWriter_TokenFormat(t *testing.T) {
...
func TestRedactingWriter_ChunkingInvariance(t *testing.T) {
	const v = "p@ss w/rd+1=2!Zk"
	values := map[string]string{"api": v}
...
func TestRedactingWriter_OverlappingOccurrences(t *testing.T) {
...
func TestRedactingWriter_NestedSecretValues(t *testing.T) {
...
// entirely by Flush.
func TestRedactingWriter_BinaryPassthrough(t *testing.T) {
...
func TestRedactingWriter_SecretAtEndOfStream(t *testing.T) {
	const v = "p@ss w/rd+1=2!Zk"
	values := map[string]string{"api": v}
	input := "prefix:" + v
...
func TestRedactingWriter_EmptyWriteIsNoOp(t *testing.T) {
...
// RedactingWriter a pure passthrough: window is 0, so nothing is ever held back.
func TestRedactingWriter_NoPatternsPassthrough(t *testing.T) {
...
// the RedactingWriter: every later Write and Flush returns the same error without
// touching the destination again.
func TestRedactingWriter_WriterErrorPoisons(t *testing.T) {
	boom := errors.New("boom")
	values := map[string]string{"s": "zz"} // window 3: a 10-byte Write must emit

	fw := &failWriter{err: boom}
	r := NewRedactingWriter(fw, NewMatcher(values))
...
func TestRedactingWriter_FlushErrorPoisons(t *testing.T) {
	boom := errors.New("boom")
...
// Using the RedactingWriter after a successful Flush is a programmer error and is
// reported as one.
func TestRedactingWriter_UseAfterFlush(t *testing.T) {
	var buf bytes.Buffer
	r := NewRedactingWriter(&buf, NewMatcher(map[string]string{"s": "zz"}))
