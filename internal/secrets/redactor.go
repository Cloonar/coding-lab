...
// Redactor is a one-shot stream filter, and touching it after the stream has
// been finalised is a programmer error.
var errUsedAfterFlush = errors.New("secrets: Redactor used after Flush")

// Redactor is an io.Writer that forwards a byte stream to an underlying
...
// Write the Redactor must decide which pending bytes can no longer be part of
...
// be covered by a match that has not already been reported: the Redactor
...
// forwarded to dst differs (held-back tail, token substitution): the Redactor
...
// Write returns (0, err) and the Redactor is poisoned — every subsequent
...
//
// Like the Matcher it owns, a Redactor is not safe for concurrent use: use
// one Redactor per stream.
type Redactor struct {
	...
	err     error // first underlying-writer error; poisons the Redactor
	flushed bool
}
...
// NewRedactor returns a Redactor forwarding redacted output to dst. The
// Redactor owns m from this point on: m must be freshly constructed or Reset,
// and nothing else may Feed or Reset it for the Redactor's lifetime.
func NewRedactor(dst io.Writer, m *Matcher) *Redactor {
	return &Redactor{dst: dst, m: m}
}
...
// an error. On a poisoned Redactor, Flush returns the poisoning error and
// writes nothing.
...
func (r *Redactor) appendPlaintext(p []byte) {
...
func (r *Redactor) apply(matches []Match) {
	if len(matches) == 0 {
		return
	}
...
func (r *Redactor) redact(mt Match) {
	last := len(r.pend) - 1
	s := r.pend[last]
	r.pend = r.pend[:last]
	if mt.Start > s.start {
...
func (r *Redactor) consume(start, end int64) {
...
func (r *Redactor) emit(limit int64) error {
	var out []byte
	i := 0
	for i < len(r.pend) {
		s := &r.pend[i]
